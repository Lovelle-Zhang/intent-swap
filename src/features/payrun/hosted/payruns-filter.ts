...
export function renderFilterBar(filter: PayRunFilter, total: number, shown: number, page?: PayRunPage): string {
  ...
  const dateField = (id: string, value: string) =>
    `<input class="datefield" type="text" inputmode="numeric" pattern="\\d{4}-\\d{2}-\\d{2}" title="Use YYYY-MM-DD" placeholder="YYYY-MM-DD" id="${id}" name="${id}" value="${escapeHtml(value)}">`;
  const dates = page
