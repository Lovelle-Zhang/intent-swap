// The hosted shell's design-system CSS + brand mark, extracted from ui.ts so the
// shell logic (hostedPage, statusBadge, nav) stays small. Behaviour is identical:
// ui.ts imports STYLE/LOGO from here and inlines them exactly as before. Visual
// language matches the ZenFix landing: control-layer / ledger identity.

export const LOGO = `<svg viewBox="0 0 40 40" aria-hidden="true"><rect x=".75" y=".75" width="38.5" height="38.5" rx="11" fill="var(--surface-2)" stroke="var(--line)"/><path d="M17.5 12.5 H13 V27.5 H17.5" fill="none" stroke="var(--signal)" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/><path d="M22.5 12.5 H27 V27.5 H22.5" fill="none" stroke="var(--signal)" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/><circle cx="20" cy="20" r="3.1" fill="var(--sandbox)"/></svg>`;
