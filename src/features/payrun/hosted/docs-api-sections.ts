...
export const API_SECTIONS: readonly DocSection[] = [
  ...
  },
  {
    ...
    <p><b>Verified execution (recommended).</b> Report <span class="mono">rail: "base-sepolia"</span> (testnet) or <span class="mono">rail: "base-mainnet"</span> (real USDC on Base) with the real <span class="mono">transactionHash</span>. To earn the <b>Verified on-chain</b> badge you must first pin the merchant&rsquo;s payout address on the <a class="link" href="/zenfix/policy">Policy</a> page — that pinned address is the trust anchor. ZenFix then reads the public chain and confirms the transaction succeeded, moved that chain&rsquo;s USDC, paid at least the authorized amount, and went to <b>that pinned address</b>; a claim it can't verify is rejected with <span class="mono">422</span>. <b>Without a pinned address for the merchant the outcome is recorded as self-reported</b>, not proof-backed — a <span class="mono">recipient</span> the agent merely declares can be set to match any public transfer, so it can't anchor the proof. You may also pass the paying wallet as <span class="mono">sender</span>; the on-chain transfer must then also have come <b>from</b> it (for x402/EIP-3009 the authorizing wallet is the transfer's <span class="mono">from</span> even when a facilitator submits the tx). Non-Base rails are always self-reported.</p>
  ...
  "verification": { "verified": true, "chain": "base-sepolia", "amountAtomic": "20000000", "recipient": "0x...", "pinnedMerchant": true } }`)}
    <p class="muted">On a self-reported outcome — a non-Base rail, or a verified rail without a pinned merchant address — <span class="mono">verification</span> is <span class="mono">{ "verified": false }</span>.</p>`,
  },
  {
    id: "poll",
